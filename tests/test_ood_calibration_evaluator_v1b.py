...
class OODCalibrationEvaluatorV1Tests(
    ...
    def test_failed_v1_provenance_anchor(
        self,
    ):
        self.assertEqual(
            MOD.FAILED_EVALUATOR_V1_COMMIT,
            "50110f0217e8fa81e4851777ee3a8dcd44c184a2",
        )

        self.assertEqual(
            MOD.file_sha256(
                MOD.FAILED_EVALUATOR_V1_SOURCE
            ),
            MOD.FAILED_EVALUATOR_V1_RAW_SHA256,
        )

    def test_legacy_pickle_dependency_import_only(
        self,
    ):
        compatibility = (
            MOD
    ...
    def test_v1b_uses_legacy_class_only_as_pickle_compatibility(
        self,
    ):
        source = MODULE_PATH.read_text()
